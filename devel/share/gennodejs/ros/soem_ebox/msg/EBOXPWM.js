// Auto-generated. Do not edit!

// (in-package soem_ebox.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class EBOXPWM {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.pwm = null;
    }
    else {
      if (initObj.hasOwnProperty('pwm')) {
        this.pwm = initObj.pwm
      }
      else {
        this.pwm = new Array(2).fill(0);
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type EBOXPWM
    // Check that the constant length array field [pwm] has the right length
    if (obj.pwm.length !== 2) {
      throw new Error('Unable to serialize array field pwm - length must be 2')
    }
    // Serialize message field [pwm]
    bufferOffset = _arraySerializer.float32(obj.pwm, buffer, bufferOffset, 2);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type EBOXPWM
    let len;
    let data = new EBOXPWM(null);
    // Deserialize message field [pwm]
    data.pwm = _arrayDeserializer.float32(buffer, bufferOffset, 2)
    return data;
  }

  static getMessageSize(object) {
    return 8;
  }

  static datatype() {
    // Returns string type for a message object
    return 'soem_ebox/EBOXPWM';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '0d644c5720c3d45355d4cb6a20aa7362';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    float32[2] pwm
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new EBOXPWM(null);
    if (msg.pwm !== undefined) {
      resolved.pwm = msg.pwm;
    }
    else {
      resolved.pwm = new Array(2).fill(0)
    }

    return resolved;
    }
};

module.exports = EBOXPWM;
